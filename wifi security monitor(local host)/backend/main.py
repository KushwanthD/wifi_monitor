...
UNIVERSAL_SCANNER_TASK = None

def is_local_agent_running() -> bool:
    global UNIVERSAL_SCANNER_TASK
    return UNIVERSAL_SCANNER_TASK is not None and not UNIVERSAL_SCANNER_TASK.done()
...
def get_local_agent_status():
    ...
    return {
        "running": running,
        "pairing_code": LOCAL_PAIRING_CODE if running else None,
        "agent_id": os.environ.get("COMPUTERNAME", "UNKNOWN").strip().upper() if running else None
    }

@app.post("/api/agent/run-local")
async def run_local_agent():
    global LOCAL_PAIRING_CODE, UNIVERSAL_SCANNER_TASK
    if is_local_agent_running():
        if not LOCAL_PAIRING_CODE:
            generate_pairing_code()
        return {"status": "already_running", "message": "Local agent is already running.", "pairing_code": LOCAL_PAIRING_CODE, "agent_id": os.environ.get("COMPUTERNAME", "UNKNOWN").strip().upper()}

    async def _callback(agent_id, report):
        class MockRequest:
            async def json(self): return report
        await receive_agent_report(MockRequest())

    agent_id = os.environ.get("COMPUTERNAME", "UNKNOWN").strip().upper()
    UNIVERSAL_SCANNER_TASK = asyncio.create_task(universal_scanner.run_scan_loop(agent_id, _callback))
    generate_pairing_code()
    return {"status": "started", "message": "Local agent launched on the backend.", "pairing_code": LOCAL_PAIRING_CODE, "agent_id": agent_id}
...
def startup_event():
    ...
    # Auto-open the application in the default web browser after a short delay
    def open_browser():
        time.sleep(1.5)
...
if __name__ == '__main__':
    import sys
    # When running with PyInstaller --noconsole, stdout and stderr are None.
    # Uvicorn tries to call isatty() on them and crashes. We mock them here.
